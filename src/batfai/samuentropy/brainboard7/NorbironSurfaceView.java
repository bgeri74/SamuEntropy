...
public class NorbironSurfaceView extends android.view.SurfaceView implements Runnable {
    ...
    private int[] CONNECTION_COLORS = new int[7];
    ...
    public static void saveNodes(){
        ...
        Log.d("gery", "Saving " + nodeBoxes.size() + " nodes.");
        for(int i = 0; i < nodeBoxes.size(); i++){
            //Log.d("gery", "Saving #" + i + " node");
                database.getWritableDatabase().execSQL(String.format("INSERT INTO positions (X, Y, type) VALUES (%d, %d, %d)", nodeBoxes.get(i).getX(),
                   nodeBoxes.get(i).getY(), nodeBoxes.get(i).getIndex()));
    ...
    private void cinit(final android.content.Context context) {

        NeuronGameActivity.canPressUpgradeBtn = false;

        this.context = context;
        nodes = new Nodes(this);
        nodeBoxes = new java.util.ArrayList<NeuronBox>();

        CONNECTION_COLORS[0] = Color.RED;
        CONNECTION_COLORS[1] = Color.BLUE;
        CONNECTION_COLORS[2] = Color.GREEN;
        CONNECTION_COLORS[3] = Color.CYAN;
        CONNECTION_COLORS[4] = Color.GRAY;
        CONNECTION_COLORS[5] = Color.MAGENTA;
        CONNECTION_COLORS[6] = Color.YELLOW;
...
database.getResult("SELECT * FROM positions");
                    ...
                    Log.d("gery", "Adding nodes");
                    try {
                        database.getLastResult().moveToFirst();
                        xCoord = database.getLastResult().getFloat(database.getLastResult().getColumnIndex("X"));
                        yCoord = database.getLastResult().getFloat(database.getLastResult().getColumnIndex("Y"));
                        index = database.getLastResult().getInt(database.getLastResult().getColumnIndex("type"));

                        NeuronBox node = (NeuronBox) nodes.get(index).clone();
                        node.setIndex(index);
                        node.setXY(xCoord, yCoord);
                        nodeBoxes.add(node);
                        ...
                        while (database.getLastResult().moveToNext()) {
                            ...
                            node = (NeuronBox) nodes.get(index).clone();
                            node.setIndex(index);
                            node.setXY(xCoord, yCoord);
                            nodeBoxes.add(node);
                        ...
                        Log.d("gery", "cursorindexoutofbounds in norbironsurfaceview");
                ...
                protected void onPostExecute(Void aVoid) {
                    ...
                    NeuronGameActivity.canPressUpgradeBtn = true;
    ...
    public void onDraw(android.graphics.Canvas canvas) {
        ...
        if (surfaceHolder.getSurface().isValid()) {
            ...
            for (NeuronBox nb : nodeBoxes) {
                ...
                paint.setColor(CONNECTION_COLORS[nb.getIndex()]);
                paint.setStrokeWidth(5);
                try {
                    for (NeuronBox box : nodeBoxes) {
                        if (box.getIndex() == nb.getIndex())
                            canvas.drawLine(nb.getX() + nb.getWidth() / 2 - startsx, nb.getY() + nb.getHeight() / 2 - startsy, box.getX() + box.getWidth() / 2 - startsx, box.getY() + box.getHeight() / 2 - startsy, paint);
                    }
                }
